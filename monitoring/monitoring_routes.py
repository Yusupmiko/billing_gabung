...
# ✅ PASTIKAN ROUTE INI JUGA ADA DAN SUDAH BENAR
@monitoring_bp.route('/get_detail_pelanggan_dlpd_hb')
@login_required
def get_detail_pelanggan_dlpd_hb():
    """Get customer details by BLTH, KDKELOMPOK with DLPD filter (including AMAN status)"""
...
# ✅ PENTING: Pastikan fungsi get_user_unitup_filter menerima override_unitup
def get_user_unitup_filter(username, override_unitup=None):
    """
    Get UNITUP filter for user
    Args:
        username: Username
        override_unitup: UNITUP from filter (for admin override)
    Returns:
        UNITUP code or None (for admin without filter)
    """
    user_info = get_user_info(username)
    ...
    # Check if admin (UP3 or Administrator)
    is_admin = unitup == 'UP3' or 'Administrator' in str(nama_ulp) or 'UP3' in str(nama_ulp).upper()
    
    if is_admin:
        # Admin: Use override if provided, otherwise None (all data)
        return override_unitup if override_unitup else None
    
    # Regular user: Always use their UNITUP
    return unitup
