...
def dashboard_ulp():
    ...
    # ✅ FIX: UP3 tidak perlu validasi UNITUP
    ...
    if request.method == 'POST' and 'file_dpm' in request.files:
        ...
        try:
            ...
            # ✅ Jika UP3 upload, pastikan ada kolom UNITUP dan beri suffix _up3
            if role == 'UP3':
                if 'UNITUP' not in df_upload.columns:
                    flash('❌ File Excel harus memiliki kolom UNITUP untuk upload oleh UP3!', 'danger')
                    return redirect(url_for('dashboard_ulp'))
                
                # Tambahkan suffix _up3
                df_upload['UNITUP'] = df_upload['UNITUP'].astype(str).str.strip() + '_up3'
                unitup_for_upload = None  # supaya tidak pakai session
                logger.info(f"✅ UP3 upload detected. UNITUP modified with _up3 suffix.")
    ...
    if request.method == 'POST' and request.form.get('action') == 'process_billing':
        # ✅ UP3 hanya proses data yang diupload UP3 (suffix _up3)
        if role == 'UP3':
            # flash('🟢 UP3 menjalankan proses billing untuk data UP3.', 'info')
            # ✅ Kirim pattern '_up3' untuk filter data UP3 saja
            df_billing, error = process_billing_advanced(blth_kini, '_up3', engine)
        else:
...
def dashboard_up3_manage(unitup):
    ...
    if request.method == 'POST' and 'file_dpm' in request.files:
        ...
        try:
            ...
            # ✅ PERBAIKAN 1: Normalisasi kolom seperti route lama
            df_upload.columns = [c.strip().upper() for c in df_upload.columns]
            
            # ✅ PERBAIKAN 2: Validasi kolom UNITUP
            if 'UNITUP' not in df_upload.columns:
                flash('❌ File Excel harus memiliki kolom UNITUP untuk upload oleh UP3!', 'danger')
                return redirect(url_for('dashboard_up3_manage', unitup=unitup))
            
            # ✅ PERBAIKAN 3: Tambahkan suffix _up3 seperti route lama
            df_upload['UNITUP'] = df_upload['UNITUP'].astype(str).str.strip() + '_up3'
            logger.info(f"✅ UP3 upload detected for {unitup}. UNITUP modified with _up3 suffix.")
            
            # ✅ PERBAIKAN 4: Pass None sebagai unitup_for_upload (seperti route lama)
    ...
    if request.method == 'POST' and request.form.get('action') == 'process_billing':
        try:
            # ✅ PERBAIKAN 5: Gunakan pattern '_up3' untuk filter data UP3
            # Atau gunakan unitup + '_up3' untuk filter spesifik ULP ini
            unitup_pattern = f"{unitup}_up3"
            df_billing, error = process_billing_advanced(blth_normalized, unitup_pattern, engine)
    ...
    # =================== GET SUMMARY ===================
    try:
        # ✅ PERBAIKAN 6: Query menggunakan unitup dengan suffix _up3
        unitup_with_suffix = f"{unitup}_up3"
        query = text("""
            SELECT 
                blth,
                COUNT(*) as total,
            ...
            LIMIT 6
        """)
        df_summary = pd.read_sql(query, engine, params={'unitup': unitup_with_suffix})
    ...
    return render_template(
        ...
        unitup=unitup,  # Tampilkan unitup tanpa suffix di UI
        role='UP3_MANAGE',  # Flag bahwa ini UP3 manage ULP
        summary=df_summary.to_dict('records') if not df_summary.empty else [],
        blth_terakhir=blth_normalized,
        back_url=url_for('dashboard_up3')  # Tombol kembali
    )
...
def update_verifikasi_single():
    try:
        ...
        if role == 'UP3':
            ...
            # ULP hanya bisa update data milik sendiri
            sql = text(f"""
                UPDATE billing 
                SET `{column}` = :value
                WHERE IDPEL = :idpel 
