...
def process_billing_advanced(blth_kini, unitup, engine):
    ...
    try:
        ...
        # --- Foto & Grafik ---
        path_foto1 = "https://portalapp.iconpln.co.id/acmt/DisplayBlobServlet1?idpel="
        path_foto2 = "https://portalapp.iconpln.co.id/acmt/DisplayBlobServlet2?idpel="  # ✅ TAMBAHAN
        path_param = "&blth="

        kroscek['FOTO_AKHIR'] = kroscek['IDPEL'].apply(
            lambda x: f"""<button class='btn btn-sm btn-success' 
                onclick="window.open('{path_foto1}{x}{path_param}{blth_kini}', '_blank', 
                'width=700,height=700,scrollbars=yes,toolbar=no'); return false;">
                <i class='bi bi-image'></i> Akhir
            </button>"""
        )

        # ✅ TAMBAHAN: FOTO_AKHIR2 dengan Servlet2
        kroscek['FOTO_AKHIR2'] = kroscek['IDPEL'].apply(
            lambda x: f"""<button class='btn btn-sm btn-primary' 
                onclick="window.open('{path_foto2}{x}{path_param}{blth_kini}', '_blank', 
                'width=700,height=700,scrollbars=yes,toolbar=no'); return false;">
                <i class='bi bi-image'></i> Akhir2
            </button>"""
        )

        kroscek['FOTO_LALU'] = kroscek['IDPEL'].apply(
            lambda x: f"""<button class='btn btn-sm btn-warning' 
                onclick="window.open('{path_foto1}{x}{path_param}{blth_lalu}', '_blank', 
                'width=700,height=700,scrollbars=yes,toolbar=no'); return false;">
                <i class='bi bi-image'></i> Lalu
            </button>"""
        )
        ...
        kroscek['FOTO_LALU2'] = kroscek['IDPEL'].apply(
            lambda x: f"""<button class='btn btn-sm btn-secondary' 
                onclick="window.open('{path_foto1}{x}{path_param}{blth_lalulalu}', '_blank', 
                'width=700,height=700,scrollbars=yes,toolbar=no'); return false;">
                <i class='bi bi-image'></i> Lalu2
            </button>"""
        )
...
# =================== PERBAIKAN AUTO-BILLING DENGAN ERROR HANDLING LEBIH BAIK ===================
@app.route('/dashboard_ulp', methods=['GET', 'POST'])
def dashboard_ulp():
    """📊 Dashboard ULP dengan Auto-Process Billing setelah Upload DPM"""
    if 'loggedin' not in session:
        return redirect(url_for('login'))
    ...
    # =================== UPLOAD DPM BARU (DENGAN AUTO-PROCESS BILLING) ===================
    if request.method == 'POST' and request.form.get('form_type') == 'upload_dpm':
        logger.info("🔵 Processing UPLOAD DPM BARU")
        ...
        try:
            ...
            flash(f'✅ Berhasil upload {count} data DPM', 'success')
            logger.info(f"✅ Upload DPM success: {count} rows")
            
            # ✅ STEP 2: AUTO-PROCESS BILLING (DENGAN ERROR HANDLING)
            try:
                logger.info(f"🔄 AUTO-PROCESSING BILLING untuk BLTH: {blth_kini}")
                
                # Tentukan UNITUP untuk proses billing
                if role == 'UP3':
                    # UP3: Proses semua UNITUP yang baru di-upload
                    unitup_list = df_upload['UNITUP'].unique().tolist()
                    logger.info(f"📊 UP3 mode: Processing {len(unitup_list)} UNITUP")
                    ...
                    for up in unitup_list:
                        try:
                            df_billing, error = process_billing_advanced(blth_kini, up, engine)
                            
                            if error:
                                logger.error(f"❌ Billing error for {up}: {error}")
                                total_failed += count
                                flash(f'⚠️ AUTO-BILLING gagal untuk {up}: {error}', 'warning')
                                continue
                            
                            if df_billing.empty:
                                logger.warning(f"⚠️ No billing data for {up}")
                                continue
                            
                            result = save_to_billing_with_trigger(df_billing, engine, username)
                            billing_results.append((up, result))
                            total_success += result['success']
                            total_failed += result['failed']
                            logger.info(f"✅ Billing {up}: {result['success']} success, {result['failed']} failed")
                            
                        except Exception as e:
                            logger.error(f"❌ Exception processing billing for {up}: {str(e)}")
                            total_failed += count
                            continue
                    
                    # Summary flash message
                    if total_success > 0:
                        flash(f'🤖 AUTO-BILLING: Berhasil {total_success} data, Gagal {total_failed} data', 'success')
                    else:
                        flash(f'⚠️ AUTO-BILLING: Semua gagal ({total_failed} data)', 'warning')
                
                else:
                    # ULP: Proses hanya UNITUP sendiri
                    df_billing, error = process_billing_advanced(blth_kini, unitup, engine)
                    
                    if error:
                        flash(f'⚠️ AUTO-BILLING gagal: {error}. Data DPM sudah tersimpan.', 'warning')
                        logger.error(f"❌ Auto-billing error: {error}")
                    elif df_billing.empty:
                        flash('ℹ️ AUTO-BILLING: Tidak ada data untuk diproses', 'info')
                    else:
                        result = save_to_billing_with_trigger(df_billing, engine, username)
                        flash(f'🤖 AUTO-BILLING: {result["message"]}', 
                              'success' if result['success'] > 0 else 'warning')
                        logger.info(f"✅ Auto-billing: {result['success']} success, {result['failed']} failed")
                
            except Exception as e:
                logger.error(f"❌ Auto-billing critical error: {str(e)}")
                logger.error(traceback.format_exc())
                flash(f'⚠️ AUTO-BILLING error: {str(e)}. Data DPM sudah tersimpan, silakan proses billing manual.', 'warning')
    ...
    # =================== UPDATE DPM KOREKSI (DENGAN AUTO-RE-PROCESS BILLING) ===================
    if request.method == 'POST' and request.form.get('form_type') == 'update_koreksi':
        logger.info("🟡 Processing UPDATE DPM KOREKSI")
        ...
        try:
            logger.info(f"🔄 Starting DPM Koreksi for {unitup} by {username} ({role})")
            ...
            # Update database
            updated = failed = 0
            affected_blth_unitup = set()  # Track BLTH+UNITUP yang terpengaruh
            ...
            with engine.begin() as conn:
                for idx, row in df_koreksi.iterrows():
                    try:
                        ...
                        if result.rowcount > 0:
                            updated += 1
                            affected_blth_unitup.add((row['BLTH'], target_unitup))
                        else:
                            failed += 1
            ...
            if updated > 0:
                ...
                # ✅ AUTO-RE-PROCESS BILLING untuk BLTH+UNITUP yang terpengaruh
                try:
                    logger.info(f"🔄 AUTO-RE-PROCESSING BILLING untuk {len(affected_blth_unitup)} kombinasi BLTH+UNITUP")
                    
                    billing_results = []
                    total_success = 0
                    
                    for blth, up in affected_blth_unitup:
                        try:
                            logger.info(f"🔄 Re-processing: BLTH={blth}, UNITUP={up}")
                            df_billing, error = process_billing_advanced(blth, up, engine)
                            
                            if error:
                                logger.error(f"❌ Re-billing error for {blth}/{up}: {error}")
                                continue
                            
                            if df_billing.empty:
                                logger.warning(f"⚠️ No billing data for {blth}/{up}")
                                continue
                            
                            result = save_to_billing_with_trigger(df_billing, engine, username)
                            billing_results.append((blth, up, result))
                            total_success += result['success']
                            logger.info(f"✅ Re-billing {blth}/{up}: {result['success']} success")
                            
                        except Exception as e:
                            logger.error(f"❌ Exception re-billing {blth}/{up}: {str(e)}")
                            continue
                    
                    # Summary
                    if total_success > 0:
                        flash(f'🤖 AUTO-RE-BILLING: Berhasil update {total_success} data billing', 'success')
                    else:
                        flash(f'⚠️ AUTO-RE-BILLING gagal. Silakan proses billing manual.', 'warning')
                    
                except Exception as e:
                    logger.error(f"❌ Auto-re-billing critical error: {str(e)}")
                    flash(f'⚠️ AUTO-RE-BILLING error: {str(e)}. Silakan proses billing manual.', 'warning')
    ...
    # =================== PROCESS BILLING MANUAL (TETAP TERSEDIA) ===================
    if request.method == 'POST' and request.form.get('action') == 'process_billing':
        logger.info("🟢 Processing BILLING (MANUAL)")
   ...
   # =================== GET 12 BULAN TERAKHIR YANG SUDAH ADA DATA ===================
    try:
        logger.info(f"🔍 Fetching uploaded months for UNITUP: {unitup}, Role: {role}")
        
        if role == 'UP3':
            # UP3: Ambil semua UNITUP
            uploaded_query = text("""
                ...
                LIMIT 12
            """)
            uploaded_months_data = pd.read_sql(uploaded_query, engine)
        else:
            # ULP: Filter by UNITUP
            uploaded_query = text("""
                SELECT DISTINCT BLTH
                FROM dpm
                WHERE UNITUP = :unitup
                ...
                LIMIT 12
            """)
            uploaded_months_data = pd.read_sql(uploaded_query, engine, params={'unitup': unitup})
        
        logger.info(f"📊 Query result: {uploaded_months_data}")
        
        # Format YYYYMM ke YYYY-MM
        uploaded_months = []
        if not uploaded_months_data.empty:
            for blth in uploaded_months_data['BLTH']:
                if blth and len(str(blth)) == 6:
                    year = str(blth)[:4]
                    month = str(blth)[4:6]
                    uploaded_months.append(f"{year}-{month}")
        
        logger.info(f"✅ Formatted months: {uploaded_months}")
        
    except Exception as e:
        logger.error(f"❌ Error getting uploaded months: {e}")
        logger.error(traceback.format_exc())
        uploaded_months = []

    # Debug: Pastikan data ter-pass ke template
    logger.info(f"📤 Passing to template - uploaded_months: {uploaded_months}")
    ...
    return render_template(
        ...
        summary=df_summary.to_dict('records') if not df_summary.empty else [],
        blth_terakhir=blth_kini,
        uploaded_months=uploaded_months  # ✅ TAMBAHKAN INI
    )
...
# =================== DASHBOARD UP3 MANAGE - HANYA TAMBAH AUTO-BILLING ===================
# ⚠️ ROUTE TIDAK DIUBAH! Hanya tambah auto-billing setelah upload
...
def dashboard_up3_manage(unitup):
    ...
    # =================== UPLOAD DPM DENGAN AUTO-BILLING ===================
    if request.method == 'POST' and 'file_dpm' in request.files:
        ...
        try:
            ...
            logger.info(f"✅ Upload DPM success: {count} rows for {unitup}")
            
            # ✅ STEP 2: AUTO-BILLING (TAMBAHAN BARU - INI YANG MISSING!)
            try:
                logger.info(f"🔄 AUTO-PROCESSING BILLING untuk {unitup}, BLTH: {blth_normalized}")
                
                df_billing, error = process_billing_advanced(blth_normalized, unitup, engine)
                
                if error:
                    flash(f'⚠️ AUTO-BILLING gagal: {error}. Data DPM sudah tersimpan.', 'warning')
                    logger.error(f"❌ Auto-billing error for {unitup}: {error}")
                elif df_billing.empty:
                    flash('ℹ️ AUTO-BILLING: Tidak ada data untuk diproses', 'info')
                else:
                    result = save_to_billing_with_trigger(df_billing, engine, username)
                    flash(f'🤖 AUTO-BILLING: {result["message"]} untuk {unitup}', 
                          'success' if result['success'] > 0 else 'warning')
                    logger.info(f"✅ Auto-billing {unitup}: {result['success']} success, {result['failed']} failed")
                
            except Exception as e:
                logger.error(f"❌ Auto-billing error for {unitup}: {str(e)}")
                logger.error(traceback.format_exc())
                flash(f'⚠️ AUTO-BILLING error: {str(e)}. Data DPM sudah tersimpan, proses billing manual jika perlu.', 'warning')
    ...
    # =================== UPDATE DPM KOREKSI DENGAN AUTO-RE-BILLING ===================
    if request.method == 'POST' and request.form.get('form_type') == 'update_koreksi':
        ...
        try:
            ...
            if updated > 0:
                ...
                # ✅ AUTO-RE-BILLING (TAMBAHAN BARU)
                try:
                    logger.info(f"🔄 AUTO-RE-BILLING untuk {len(affected_blth)} BLTH")
                    
                    total_success = 0
                    for blth in affected_blth:
                        try:
                            df_billing, error = process_billing_advanced(blth, unitup, engine)
                            
                            if error or df_billing.empty:
                                continue
                            ...
                            logger.error(f"❌ Re-billing error {blth}: {str(e)}")
                            continue
                    ...
                    logger.error(f"❌ Auto-re-billing error: {str(e)}")
                    flash('💡 Proses ulang billing manual untuk refresh', 'info')
    ...
    # =================== GET 12 BULAN TERAKHIR YANG SUDAH ADA DATA ===================
    try:
        logger.info(f"🔍 [UP3_MANAGE] Fetching uploaded months for UNITUP: {unitup}")
        
        # Query untuk UNITUP spesifik yang sedang dikelola UP3
        uploaded_query = text("""
            SELECT DISTINCT BLTH
            FROM dpm
            WHERE UNITUP = :unitup
        ...
        logger.info(f"✅ [UP3_MANAGE] Formatted months: {uploaded_months}")
    ...
    logger.info(f"📤 [UP3_MANAGE] Passing to template - uploaded_months: {uploaded_months}")
        
    # # ✅ ROUTE TIDAK BERUBAH - TETAP MENGGUNAKAN STRUKTUR YANG SUDAH ADA
    # return render_template(
    #     'dashboard_ulp.html',
    ...
    # )
    # Render template dashboard_ulp.html dengan data tambahan
    return render_template(
        'dashboard_ulp.html',
        nama=f"{nama} (Dikelola oleh {username})",
        unitup=unitup,
        ...
        back_url=url_for('dashboard_up3'),
        uploaded_months=uploaded_months,  # ✅ TAMBAHKAN INI
        is_up3_manage=True  # ✅ FLAG UNTUK TEMPLATE
    )
...
@app.route('/edit_user/<int:id_user>', methods=['POST'])
def edit_user(id_user):
    """
    ✏️ Edit user yang sudah ada
    """
    if 'loggedin' not in session or session.get('role') != 'UP3':
        flash('Akses ditolak', 'danger')
        return redirect(url_for('login'))
    ...
    if not all([unitup, username]):
        flash('UNITUP dan Username harus diisi', 'danger')
        return redirect(url_for('kelola_user'))
    ...
    try:
        with engine.begin() as conn:
            ...
            if check > 0:
                flash('Username sudah digunakan oleh user lain!', 'warning')
                return redirect(url_for('kelola_user'))
            
            # Check if unitup already used by another user (kecuali role UP3)
            if role == 'ULP':
                check_unitup = conn.execute(
                    text("""
                        SELECT COUNT(*) FROM tb_user 
                        WHERE unitup = :unitup AND id_user != :id_user
                    """),
                    {'unitup': unitup, 'id_user': id_user}
                ).scalar()
                
                if check_unitup > 0:
                    flash('UNITUP sudah digunakan oleh user lain!', 'warning')
                    return redirect(url_for('kelola_user'))
            
            # Update user data
            if password:
                # If password is provided, update with new hashed password
                import hashlib
                hashed_pw = hashlib.sha256(password.encode()).hexdigest()
                ...
                # If password is not provided, don't update password
                conn.execute(text("""
                    UPDATE tb_user 
                    SET unitup = :unitup,
                        nama_ulp = :nama_ulp,
        ...
        flash(f'Gagal mengupdate user: {str(e)}', 'danger')
...
def download_excel():
    try:
        ...
        # ===================== 🔹 Hapus Kolom yang Tidak Diperlukan =====================
        # ⭐ TAMBAHAN: Hapus kolom GRAFIK dan FOTO_3BLN
        columns_to_exclude = ['updated_by', 'created_at', 'id', 'GRAFIK', 'FOTO_3BLN']
        df = df.drop(columns=[col for col in columns_to_exclude if col in df.columns], errors='ignore')
        ...
        # # Identifikasi kolom yang berisi foto/hyperlink
        # # ⭐ PERBAIKAN: Hapus GRAFIK dan FOTO_3BLN dari list foto_columns
        # foto_columns = ["FOTO_AKHIR", "FOTO_LALU", "FOTO_LALU2"]
        # foto_indexes = [i for i, col in enumerate(df.columns) if col.upper() in foto_columns]

        # Identifikasi kolom yang berisi foto/hyperlink
        # ✅ TAMBAHKAN FOTO_AKHIR2
        foto_columns = ["FOTO_AKHIR", "FOTO_AKHIR2", "FOTO_LALU", "FOTO_LALU2"]
        foto_indexes = [i for i, col in enumerate(df.columns) if col.upper() in foto_columns]
...
#         # ===================== 🔹 Identifikasi Kolom Foto =====================
#         # ⭐ PERBAIKAN: Hapus GRAFIK dan FOTO_3BLN dari list
#         # foto_columns = ["FOTO_AKHIR", "FOTO_LALU", "FOTO_LALU2"]
#         # ✅ TAMBAHKAN FOTO_AKHIR2
#         foto_columns = ["FOTO_AKHIR", "FOTO_AKHIR2", "FOTO_LALU", "FOTO_LALU2"]
#         foto_indexes = [i for i, col in enumerate(df.columns) if col.upper() in foto_columns]
...
def download_billing():
    try:
        ...
        username = session.get('username', '').lower()
        user_role = session.get('role', 'ULP')
        user_unitup = session.get('unitup', '')
        
        # Parameter dari request
        blth = request.args.get("blth")
        unitup_filter = request.args.get("unitup_filter", "")  # Dari frontend (untuk UP3)
        
        if not blth:
            return jsonify({"error": "Parameter blth wajib"}), 400

        # ✅ Bangun WHERE clause dengan filter UNITUP
        where_conditions = ["blth = :blth"]
        where_conditions.append("DLPD_HITUNG IS NOT NULL")
        where_conditions.append("TRIM(DLPD_HITUNG) <> ''")
        
        params = {"blth": blth}
        
        # ✅ Filter berdasarkan role
        if user_role == 'UP3':
            # UP3: bisa pilih UNITUP atau download semua
            if unitup_filter:
                where_conditions.append("UNITUP = :unitup")
                params["unitup"] = unitup_filter
                logger.info(f"📥 UP3 download UNITUP: {unitup_filter}")
            else:
                logger.info(f"📥 UP3 download semua UNITUP")
        
        elif user_role == 'ULP':
            # ULP: HANYA download data UNITUP sendiri
            if not user_unitup:
                return jsonify({"error": "User tidak memiliki UNITUP"}), 403
            
            where_conditions.append("UNITUP = :unitup")
            params["unitup"] = user_unitup
            logger.info(f"📥 ULP download UNITUP: {user_unitup}")
        ...
        logger.info(f"📊 Download query params: {params}")
        df = pd.read_sql(query, engine, params=params)
        ...
        # ✅ Nama file dengan UNITUP
        unitup_name = params.get('unitup', 'ALL_UP3')
        filename = f"billing_{unitup_name}_{blth}.xlsx"
        
        logger.info(f"✅ Download file: {filename}, Rows: {len(df)}")
        ...
        logger.error(f"❌ Error download billing: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({"error": f"Gagal export Excel: {str(e)}"}), 500
...
# =================== CEK FOTO PELANGGAN - SESUAI KODE ASLI ===================
@app.route('/cek_foto', methods=['GET', 'POST'])
def cek_foto():
    """
    📸 Halaman untuk cek foto pelanggan PLN
    HANYA Servlet1 dan Servlet7 (seperti kode asli)
    """
    if 'loggedin' not in session:
        return redirect(url_for('login'))
    ...
    unitup = session.get('unitup')
    role = session.get('role', 'ULP')
    nama = session.get('nama_ulp', 'User')
    ...
    # Jika POST, proses input
    if request.method == 'POST':
        idpel = request.form.get('idpel')
        blth_kini = request.form.get('blth')
        
        # Validasi input
        if not idpel or not blth_kini:
            flash('⚠️ IDPEL dan BLTH harus diisi!', 'warning')
            return redirect(url_for('cek_foto'))
        ...
        try:
            ...
            logger.error(f"Error calculating BLTH: {e}")
            flash('❌ Format BLTH tidak valid!', 'danger')
            return redirect(url_for('cek_foto'))
        
        # Generate URLs - HANYA Servlet1 (Meteran) dan Servlet7 (Rumah)
        base_url = 'https://portalapp.iconpln.co.id/acmt'
        
        links = {
            # Servlet1 - Foto Meteran
            'FOTO_KINI_1': f'{base_url}/DisplayBlobServlet1?idpel={idpel}&blth={blth_kini}',
            'FOTO_LALU_1': f'{base_url}/DisplayBlobServlet1?idpel={idpel}&blth={blth_lalu}',
            'FOTO_LALU2_1': f'{base_url}/DisplayBlobServlet1?idpel={idpel}&blth={blth_lalu2}',
            
            # Servlet7 - Foto Rumah
            'FOTO_KINI_7': f'{base_url}/DisplayBlobServlet7?idpel={idpel}&blth={blth_kini}',
            'FOTO_LALU_7': f'{base_url}/DisplayBlobServlet7?idpel={idpel}&blth={blth_lalu}',
            'FOTO_LALU2_7': f'{base_url}/DisplayBlobServlet7?idpel={idpel}&blth={blth_lalu2}',
        }
        
        logger.info(f"✅ Foto URLs generated for IDPEL: {idpel}, BLTH: {blth_kini}")
...
# =================== API ENDPOINT UNTUK CEK FOTO (AJAX) ===================
@app.route('/api/get_foto_urls', methods=['POST'])
def get_foto_urls():
    """
    🔌 API endpoint untuk mendapatkan URL foto via AJAX
    """
    if 'loggedin' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    try:
        data = request.get_json()
        idpel_input = data.get('idpel', '').strip()
        blth_input = data.get('blth', '').strip()
        
        if not idpel_input or not blth_input:
            return jsonify({'error': 'IDPEL dan BLTH harus diisi'}), 400
        
        # Normalisasi
        idpel = normalize_idpel(idpel_input)
        blth_kini = normalize_blth(blth_input)
        ...
        urls = {
            'kini': {
                'meteran': f'{base_url}/DisplayBlobServlet1?idpel={idpel}&blth={blth_kini}',
                'rumah': f'{base_url}/DisplayBlobServlet7?idpel={idpel}&blth={blth_kini}',
                'foto2': f'{base_url}/DisplayBlobServlet2?idpel={idpel}&blth={blth_kini}',
                'foto3': f'{base_url}/DisplayBlobServlet3?idpel={idpel}&blth={blth_kini}',
            },
            'lalu': {
                'meteran': f'{base_url}/DisplayBlobServlet1?idpel={idpel}&blth={blth_lalu}',
                'rumah': f'{base_url}/DisplayBlobServlet7?idpel={idpel}&blth={blth_lalu}',
                'foto2': f'{base_url}/DisplayBlobServlet2?idpel={idpel}&blth={blth_lalu}',
                'foto3': f'{base_url}/DisplayBlobServlet3?idpel={idpel}&blth={blth_lalu}',
            },
            'lalu2': {
                'meteran': f'{base_url}/DisplayBlobServlet1?idpel={idpel}&blth={blth_lalu2}',
                'rumah': f'{base_url}/DisplayBlobServlet7?idpel={idpel}&blth={blth_lalu2}',
                'foto2': f'{base_url}/DisplayBlobServlet2?idpel={idpel}&blth={blth_lalu2}',
                'foto3': f'{base_url}/DisplayBlobServlet3?idpel={idpel}&blth={blth_lalu2}',
            }
        }
        ...
        logger.error(f"Error in get_foto_urls: {str(e)}")
        return jsonify({'error': str(e)}), 500
